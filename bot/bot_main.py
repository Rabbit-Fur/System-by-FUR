...
async def on_ready():
    ...
    if Config.DISCORD_GUILD_ID:
        ...
        try:
            ...
            log.info(f"✅ Synced {len(synced)} commands to guild {Config.DISCORD_GUILD_ID}")
        except discord.errors.Forbidden:
            log.error("❌ Missing permission to sync commands.")
        except Exception as e:
            log.error(f"❌ Failed to sync commands.", exc_info=e)
    else:
        log.warning("⚠️ DISCORD_GUILD_ID not set. Slash commands may sync slowly.")
...
async def on_app_command_error(interaction: discord.Interaction, error: discord.app_commands.AppCommandError):
    try:
        if isinstance(error, discord.app_commands.CommandOnCooldown):
            await interaction.response.send_message(f"⏳ Cooldown. Retry in {error.retry_after:.2f}s.", ephemeral=True)
        elif isinstance(error, discord.app_commands.CheckFailure):
            await interaction.response.send_message("🚫 No permission.", ephemeral=True)
        else:
            log.error(f"Slash command error: {interaction.command}", exc_info=error)
