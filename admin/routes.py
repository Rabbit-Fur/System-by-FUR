...
def check_session_timeout():
    if "admin_logged_in" in session:
        ...
        try:
            ...
            timeout = current_app.config.get("PERMANENT_SESSION_LIFETIME", timedelta(minutes=30))
...
def join_event(event_id):
    return redirect(request.referrer or url_for("public.view_event", event_id=event_id))
...
def upload_file():
    ...
    if allowed_file(file.filename):
        ...
        try:
            ...
            flash(_("Upload successful: %(filename)s", filename=final_name), "success")
        except Exception as e:
