...
def get_event_by_id(event_id):
    ...
    try:
        conn = get_db()
        event = conn.execute(
            "SELECT id, title, description, event_time, role, recurrence FROM events WHERE id = ?", (event_id,)
        ).fetchone()
        if event is None:
...
def get_hall_of_fame():
    ...
    try:
        conn = get_db()
        query = (
            "SELECT id, username, honor_title, month, poster_url, created_at FROM hall_of_fame ORDER BY created_at DESC"
        )
